...
encoderOutput = Encoder_MLP(encoderInput, feedback_bits)
...
checkpointer = ModelCheckpoint(filepath='best_49sep3_3v1_autoencoder.h5', verbose=1, save_best_only=True)
scheduler = tf.keras.callbacks.LearningRateScheduler(scheduler)
earlystop_callback = EarlyStopping(monitor='val_loss', patience=50)

#model training
autoencoderModel.fit(x=x_train_3, y=x_train_3, batch_size=64, epochs=200,
                     callbacks=[earlystop_callback, scheduler,checkpointer],
                     verbose=2, validation_data=(x_val_3,x_val_3))
# Save models of encoder and decoder
encoder.save('49sep3_3v1_encoder_cnn.h5')
decoder.save('49sep3_3v1_decoder_t6.h5')